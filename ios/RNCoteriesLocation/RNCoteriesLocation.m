...
@interface RCTCoteriesLocationRequest : NSObject
...
@implementation RCTCoteriesLocationRequest
...
@interface RNCoteriesLocation () <RCTBridgeDelegate, CLLocationManagerDelegate>

@end

@implementation RNCoteriesLocation
{
	CLLocationManager *_locationManager;
	NSDictionary<NSString *, id> *_lastLocationEvent;
	NSDictionary<NSString *, id> *_lastHeadingEvent;
	NSMutableArray<RCTCoteriesLocationRequest *> *_pendingRequests;
	BOOL _observingLocation;
	RCTLocationOptions _observerOptions;
	NSDate *lastHeadingCall;
}
...
{
	RCTCoteriesLocationRequest *request = timer.userInfo;
...
RCT_EXPORT_METHOD(startObserving:(RCTLocationOptions)options)
{
	...
	for (RCTCoteriesLocationRequest *request in _pendingRequests) {
...
RCT_EXPORT_METHOD(getCurrentHeading:(RCTLocationOptions)options
	...
	// Check if previous recorded heading exists and is good enough
	//	if (_lastHeadingEvent &&
	...
	if (_lastHeadingEvent) {
		successBlock(@[_lastHeadingEvent]);
		return;
	}
									...
									errorCallback:(RCTResponseSenderBlock)errorBlock)
{
	...
	// Create request
	RCTCoteriesLocationRequest *request = [RCTCoteriesLocationRequest new];
...
- (void)locationManager:(CLLocationManager *)manager didUpdateHeading:(CLHeading *)newHeading {
	...
	_lastHeadingEvent = @{@"heading": @(heading)};

	//  NSLog(@"heading: %f", _lastHeadingEvent);
	if(!lastHeadingCall || ([[NSDate date] timeIntervalSinceDate:lastHeadingCall]) >= 1) {
		lastHeadingCall = [NSDate date];
		[self sendEventWithName:@"headingDidChange" body:_lastHeadingEvent];
	}
